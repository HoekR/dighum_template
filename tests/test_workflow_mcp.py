...
|------|-------|-----------|--------|
| **0** | [STEP0-repo](plans/steps/STEP0-repo.md) | `data_io.check` passes | ☐ |
| **1** | [STEP1-language](plans/steps/STEP1-language.md) | Parquet OK | ☑ |
| **3a** | [STEP3a-phase-a](plans/steps/STEP3a-phase-a-yearly.md) | Ribbon + PPC | ☐ |
| **6** *(opt)* | [STEP6-phase-d](plans/steps/STEP6-phase-d.md) | Gap curves | ☐ |
"""
...
| Step | Guide | Done when | Status |
|------|-------|-----------|--------|
| **0** | [STEP0-repo](docs/steps/STEP0-repo.md) | ok | ☐ |
"""
...
def test_find_current_step(project_dir: Path, mcp_path) -> None:
    ...
    assert current["guide_path"] == "plans/steps/STEP0-repo.md"
...
def test_list_step_guides_empty_project(tmp_path: Path, mcp_path) -> None:
    from workflow_mcp.steps import list_step_guides

    project = tmp_path / "thin"
    project.mkdir()
    assert list_step_guides(project) == []


def test_list_step_guides_legacy_docs_steps(tmp_path: Path, mcp_path) -> None:
    from workflow_mcp.steps import list_step_guides

    project = tmp_path / "legacy"
    project.mkdir()
    (project / "AGENTS.md").write_text("# Agents\n", encoding="utf-8")
    (project / "PLAN.md").write_text(LEGACY_PLAN, encoding="utf-8")
    steps = project / "docs" / "steps"
    steps.mkdir(parents=True)
    (steps / "STEP0-repo.md").write_text("# Step 0\n", encoding="utf-8")

    rows = list_step_guides(project)
    assert len(rows) == 1
    assert rows[0]["path"] == "docs/steps/STEP0-repo.md"
...
def test_read_step_guide(project_dir: Path, mcp_path) -> None:
    ...
    assert guide["path"] == "plans/steps/STEP3a-phase-a-yearly.md"
