...
| `get_iteration_advice(concern_id)` | Before retrying the same metric/prompt loop — needs `plans/iteration.toml` |
| `record_iteration_attempt(...)` | After a pass — append to `plans/iteration_log.jsonl` only |
| `get_iteration_config()` | List stages/concerns/facts from iteration.toml |

CLI (same package): `workflow-orchestrator advise|record|show-config`.

Example config: `dighum_template/packages/workflow_mcp/examples/iteration.toml` → copy to `plans/iteration.toml`.

## Rules

- MCP mirrors `AGENTS.md` — **one step per chat**; never "execute the whole plan".
- Agents advise and edit; **user runs terminal** by default.
- Tick `PLAN.md` yourself when **Done when** passes. Iteration tools write **only** `plans/iteration_log.jsonl`, never PLAN checkboxes.
- Optional PLAN write tools may exist in the server for convenience — **human ticks remain authoritative**.
...
| **should I keep iterating on X?** | `get_iteration_advice("X")` |
